...
| `--enable ID,...` / `--disable ID,...` | Run a subset of checkers. Unknown ids are dropped; an emptied set falls back to all. Opt-in checkers (`stale-doc-ref`, `stale-contract-ref`, `ghost-export`, `stale-entrypoint`, `stale-mock-ref`, `phantom-package`, `stale-cli-ref`) run only when named. |
