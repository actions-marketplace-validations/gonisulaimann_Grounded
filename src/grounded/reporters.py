...
def format_terminal(findings: list[Finding], n_files: int, root: str, use_color: bool = True) -> str:
    ...
    lines: list[str] = []
    if not findings:
        head = f"grounded: all beliefs check out — {n_files} file(s) scanned, 0 findings."
...
<header><div><h1>grounded <small>epistemic lint report</small></h1>
...
<footer>Deterministic &amp; offline — every finding carries claim + evidence + fix. Generated by grounded.</footer>
</div>
<script>
